...
describe("ChannelGroupSchema", () => {
  ...
  it("validates a valid group with multiple channelIds", () => {
    const result = ChannelGroupSchema.safeParse({
      ...
      channelIds: ["CBS", "CBSNewsOnline", "CBSEveningNews"],
      enabled: true,
    });
    expect(result.success).toBe(true);
  });
...
describe("ChannelGroupArraySchema", () => {
  ...
  it("rejects non-array input", () => {
    expect(ChannelGroupArraySchema.safeParse("not-an-array").success).toBe(
      false,
    );
  });
...
describe("Extension messages", () => {
  describe("GetAllMessageSchema", () => {
    it("validates get_all", () => {
      expect(GetAllMessageSchema.safeParse({ action: "get_all" }).success).toBe(
        true,
      );
    });

    it("rejects wrong action", () => {
      expect(GetAllMessageSchema.safeParse({ action: "get" }).success).toBe(
        false,
      );
    });
  });
  ...
  describe("SetMessageSchema", () => {
    ...
    it("rejects missing enabled", () => {
      expect(
        SetMessageSchema.safeParse({ action: "set", groupId: "cnn" }).success,
      ).toBe(false);
    });
  });
  ...
  describe("SetAllMessageSchema", () => {
    it("validates set_all message", () => {
      expect(
        SetAllMessageSchema.safeParse({ action: "set_all", enabled: true })
          .success,
      ).toBe(true);
    });
  });
  ...
  describe("ExtensionMessageSchema (discriminated union)", () => {
    it("accepts get_all", () => {
      expect(
        ExtensionMessageSchema.safeParse({ action: "get_all" }).success,
      ).toBe(true);
    });
    ...
    it("rejects unknown action", () => {
      expect(
        ExtensionMessageSchema.safeParse({ action: "unknown" }).success,
      ).toBe(false);
    });
  });
});
...
describe("TabUpdate schemas", () => {
  it("validates set_all update", () => {
    expect(
      SetAllUpdateSchema.safeParse({ type: "set_all", enabled: true }).success,
    ).toBe(true);
  });

  it("validates set update", () => {
    expect(
      SetUpdateSchema.safeParse({ type: "set", groupId: "cnn" }).success,
    ).toBe(true);
  });
  ...
  describe("TabUpdateSchema (discriminated union)", () => {
    it("accepts set_all", () => {
      expect(
        TabUpdateSchema.safeParse({ type: "set_all", enabled: false }).success,
      ).toBe(true);
    });
    ...
    it("rejects unknown type", () => {
      expect(TabUpdateSchema.safeParse({ type: "unknown" }).success).toBe(
        false,
      );
    });
  });
