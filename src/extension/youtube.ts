...
(function () {
  ...
  const searchFormInput =
    searchForm?.querySelector<HTMLInputElement>("#search");
  ...
  function getGroups(callback?: () => void): void {
    ...
    chrome.runtime.sendMessage({ action: "get_all" }, (values: unknown) => {
      if (!Array.isArray(values)) {
        log.warn("Received non-array response from background", {
          type: typeof values,
        });
        return;
      }
      ...
      blockedDisplayNames = enabledGroups.map((g) => g.name.toLowerCase());

      log.info("Blocked IDs and names", {
        idCount: blockedChannelIds.length,
        nameCount: blockedDisplayNames.length,
      });
      log.debug("Blocked channel IDs sample", {
        ids: blockedChannelIds.slice(0, 20),
      });
      log.debug("Blocked display names sample", {
        names: blockedDisplayNames.slice(0, 20),
      });
      callback?.();
    });
  }
  ...
  searchFormInput?.addEventListener("keyup", (event: KeyboardEvent) => {
    if (event.key === "Enter") {
  ...
  function addDailyTopLink(): boolean {
    ...
    const isActive = curSearch === "?search_query=youtube&sp=CAMSBAgCEAE%253D";
    ...
    svgEl.setAttribute(
      "stroke",
      isActive ? "#fc0d1b" : isDark ? "#aaa" : "#909090",
    );
    ...
    const poly1 = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "polyline",
    );
    poly1.setAttribute("points", "22 7 13.5 15.5 8.5 10.5 2 17");
    const poly2 = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "polyline",
    );
  ...
  function waitForVideoResults(): void {
    ...
    for (const el of channelNameEls) {
      ...
      if (
        blockedChannelIds.indexOf(text) > -1 ||
        blockedDisplayNames.indexOf(text) > -1
      ) {
        const parent = el.closest(
          "ytd-video-renderer, ytd-compact-video-renderer",
        );
    ...
    const MutationObserver =
      ...
      (
        window as unknown as {
          WebKitMutationObserver?: typeof window.MutationObserver;
        }
  ...
  const interval = window.setInterval(() => {
    ...
    if (targets.length && !observingTargetNode) {
      ...
      log.info("YouTube app content element found, setting up observer", {
        attempt: attempts,
      });
    ...
    if (attempts > 20 && !addedDailyTopLink) {
      log.warn(
        "Could not find sidebar after 20 attempts, giving up on daily link",
      );
    }
  }, 100);
